...
struct DirectionalLight : public LightSource {
    ...
    DirectionalLight(const dvec3& direction, const color& lightColor)
        : LightSource(lightColor), dir(glm::normalize(direction)) {
    }
    ...
    virtual color illuminate(const dvec3& interceptWorldCoords,
        ...
        const Material& material,
        const Frame& eyeFrame,
        bool inShadow) const override;

    virtual bool pointIsInAShadow(const dvec3& intercept,
        const dvec3& normal,
        const vector<VisibleIShapePtr>& objects,
        const Frame& eyeFrame) const override;
