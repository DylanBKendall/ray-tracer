...
double min(double A, double B, double C) {
	double min = glm::min(A, B);
	return glm::min(C, min);
}
...
double max(double A, double B, double C) {
	double max = glm::max(A, B);
	return glm::max(C, max);
}
...
double distanceBetween(double x1, double y1, double x2, double y2) {
	return distanceFromOrigin(x1 - x2, y1 - y2);
}
...
double areaOfTriangle(double x1, double y1, double x2, double y2, double x3, double y3) {
	double length1 = distanceBetween(x1, y1, x2, y2);
	double length2 = distanceBetween(x1, y1, x3, y3);
	double length3 = distanceBetween(x2, y2, x3, y3);

	return areaOfTriangle(length1, length2, length3);
}
...
dvec2 pointOnCircle(const dvec2& center, double R, double angleRads) {
	...
	return dvec2(center.x + R * x, center.y + R * y);
}
...
double directionInRadians(const dvec2& referencePt, const dvec2& targetPt) {
	return directionInRadians(targetPt - referencePt);
}
...
double directionInRadians(const dvec2& targetPt) {
	return normalizeRadians(std::atan2(targetPt.y, targetPt.x));
}
...
double directionInRadians(double x1, double y1, double x2, double y2) {
	return directionInRadians(glm::dvec2(x2 - x1, y2 - y1));
}
...
double map(double x, double fromLo, double fromHi, double toLow, double toHigh) {
	if (fromHi == fromLo || toHigh == toLow) return -1;
...
vector<double> quadratic(double A, double B, double C) {
	...
	if (discriminant >= 0) result.push_back((-B - sqrt(discriminant)) / (2 * A));
...
int quadratic(double A, double B, double C, double roots[2]) {
	...
	int rootCount = (int) result.size();

	if (rootCount > 0) roots[0] = result[0];
	if (rootCount > 1) roots[1] = result[1];
...
double areaOfTriangle(const dvec3& pt1, const dvec3& pt2, const dvec3& pt3) {
	return areaOfParallelogram((pt2 - pt1), (pt3 - pt1)) / 2.0;
}
...
dmat3 multiplyMatrices(const vector<dmat3>& M) {
    dmat3 productMatrix; // This is the identity matrix
 ...
 * @param	matrices	A vector of matrices to process. Assume the vector has length > 0.
 * @param	x	The vertex to process.
 * @return	Returns the result of M[0]*M[1]*...M[n-1]*x
 */

dvec3 multiplyMatricesAndVertex(const vector<dmat3>& matrices, const dvec3& x) {
	/* CSE 386 - todo  */
	return multiplyMatrices(matrices) * x;
}
...
vector<dvec3> multiplyMatrixAndVertices(const dmat3& M, const vector<dvec3>& verts) {
	...
	for (auto& vertex : verts) result.push_back(M * vertex);
	return result;
}
